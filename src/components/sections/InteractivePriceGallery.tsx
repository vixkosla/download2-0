...
const ICON_GROUPS = [
  ...
  },
  {
    room: 'ПРИХОЖАЯ',
    icons: [
      { name: '9 прихожая без', closed: '/app/9-прихожая-закрыта-без.png', open: '/app/9-прихожая-открыта-без.png', price: 2800, small: false, large: false, offsetY: 0, width: 120, height: 160 },
      { name: '9 прихожая с', closed: '/app/9-прихожая-закрыта-с.png', open: '/app/9-прихожая-открыта-с.png', price: 3200, small: false, large: false, offsetY: 0, width: 120, height: 160 },
    ],
  },
    {
    room: '',
    icons: [
      { name: '8 диван', closed: '/app/8-диван.png', price: 7000, small: false, large: false, offsetY: 0, width: 200, height: 120 },
      { name: '18 гостиная', closed: '/app/18-гостиная-закрыта-с.png', open: '/app/18-гостиная-открыта-с.png', price: 8000, small: false, large: false, offsetY: 0, width: 220, height: 160 },
    ],
  },
];
...
const GalleryIcon: React.FC<{
  ...
  width?: number;
  height?: number;
}> = ({ name, closed, open, price, onHover, large, small, offsetY, width, height }) => {
  const [hovered, setHovered] = React.useState(false);
  React.useEffect(() => {
  ...
  return (
    <div
      ...
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      style={{ cursor: open ? 'pointer' : 'default', width: ICON_WIDTH, height: ICON_HEIGHT, marginTop: offsetY || 0 }}
    >
      <div
        style={{
          width: ICON_WIDTH,
      ...
      {(open ? hovered : hovered) && (
        <motion.div
          ...
          animate={hovered ? { opacity: 1, scale: 1, x: '-50%', y: 0 } : { opacity: 0, scale: 0, x: '-50%', y: '-50%' }}
